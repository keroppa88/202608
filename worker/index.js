...
/* テクニカルの文面。ページの「AI分析用テキスト出力」は、これを /prompt から
   読んで .txt の頭に付ける。文面はここ1箇所にしかない */
...
3. 短期のトレンド … 直近の動き（tuned.recent。何日ぶん並んでいるかはその時
   次第。日付を見て、何日ぶんを見たのか文章の中で言う）。chg5 / chg10 / dev0 /
   rsi / stK / stD / macdH / macdHR / bbB / run / gap / range
4. 指標ごとの状態と経緯 … tuned.back の 20/40/60/80/100/150/200日前と今を比べて、
...
- like.hits の dist は今との近さ（0 に近いほど似ている）。fwd はその日から
  5/10/20/60日後の騰落率%。同じ年からは3件までしか入れていない
...
// GitHubのcronと同じUTC時刻。10分ごとの1本のCron Triggerから対象だけ起動する。
...
export default {
  ...
  async fetch(request, env) {
    ...
    // テクニカルの文面を返すだけの口。ページの「AI分析用テキスト出力」が読む。
    // Gemini も GitHub も呼ばないので、回数は数えない
    if (path === "/prompt") {
      if (request.method !== "GET") return reply(405, { error: "GET のみ" }, cors);
      return new Response(TECH_RULES, {
        status: 200,
    ...
    if (payload && payload.kind === "market") {
      if (!payload.asOf || !payload.japanMarket || !payload.indices) {
        return reply(400, { error: "asOf / japanMarket / indices が要る" }, cors);
      }
      rules = MARKET_RULES;
    ...
    try {
      ...
      if (e && e.code === "API_LIMIT") {
        return reply(429, {
          code: "API_LIMIT",
            error: "keroppa自腹によるAPI利用料が月額400円の上限に達したため、月内は利用できません。「AI分析用プロンプト出力」は無料なので、そちらを利用してください。"
        }, cors);
      }
      return reply(502, { error: String(e && e.message ? e.message : e) }, cors);
    }
